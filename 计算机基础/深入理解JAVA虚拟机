...
Garbage First：将连续的JAVA堆分成大小相同的多个区域，每个区域都可以根据需要扮演新生代，老年代和幸存区。有一类特殊的Humonogous区域专门用于存储大对象。G1将区域作为回收的最小单元，所以可预测停顿时间。G1整体来看是标记+整理，局部区域之间是标记+复制。
 ...
 - 卡表：每个 Region会维护一个哈希结构的卡表，记录了我指向谁和谁指向我
...
- 常量池：常量池中有字段量（final 修饰的常量）以及符号引用（如类全限定名等）
...
- 逃逸分析：分析对象的动态作用域，看是否逃离方法或者线程。如果逃逸程度低可以做不同的优化，如栈上分配，标量替换（创建对象的时候不去创建对象，而是创建一些基本类型作为替换），铜鼓并消除（没有竞争则消除锁）
